...
export default function CompanyDetailPage() {
      ...
      try {
        ...
        if (!companyData) {
          ...
          while (!found && page <= 100) { // Safety limit of 100 pages
            const response = await privateApiCall<{ companies: any[], pagination: any }>(`/admin/companies?page=${page}&limit=${limit}`);
            
            // Find the company by ID (handle both _id and id formats)
            companyData = response.companies.find((c: any) => {
  ...
  const handleCompanyUpdated = async () => {
    ...
    try {
      ...
      // Fetch updated company from API
      let companyData: any = null;
      let page = 1;
      const limit = 100;
      
      while (!companyData && page <= 100) {
        const response = await privateApiCall<{ companies: any[], pagination: any }>(`/admin/companies?page=${page}&limit=${limit}`);
