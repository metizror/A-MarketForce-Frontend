...
export default function DashboardLayout({
  ...
  // Handle logout - clear both localStorage and sessionStorage
  const handleLogout = () => {
    // Clear all storage
  ...
  const getActiveView = () => {
    ...
    if (pathname.startsWith('/approve-requests')) return 'approve-requests';
      ...
      <div className="flex-1 flex flex-col overflow-hidden">
        ...
        <div className="flex-1 flex overflow-hidden">
          ...
          {(() => {
            ...
            const listOnlyPages = ['dashboard', 'settings', 'activity', 'import', 'approve-requests'];
            ...
            return (
              <main className={`flex-1 overflow-hidden relative ${shouldAddPadding ? 'p-6' : ''}`}>
                <div className="absolute inset-0 overflow-hidden pointer-events-none">
                  <div className={`absolute -top-40 -right-40 w-96 h-96 bg-gradient-to-br rounded-full blur-3xl ${
                    role === 'superadmin' 
                      ...
                      : 'from-red-200/30 via-red-100/20 to-transparent'
                  }`}></div>
                  ...
                  <div className={`absolute top-0 left-0 right-0 h-1 bg-gradient-to-r from-transparent to-transparent ${
                    role === 'superadmin' ? 'via-orange-400/30' : 'via-red-400/30'
                  }`}></div>
                </div>
